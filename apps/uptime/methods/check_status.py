from apps.uptime.models import uptime_status, uptime_time_series_data
...
class status(object):
    ...
    def check(self, *args, **kwargs):
        ...
        is_data_loaded = uptime_status.objects.all().count()

        if is_data_loaded == 0:
            return {"status":"UNKNOWN"}

        isDown = uptime_status.objects.filter(status="down").count()
        ...
        if isDown == 0:
            ...
            dc_list = uptime_status.objects.values_list("DataCenter", flat=True).distinct("DataCenter").order_by("DataCenter").all()
            ...
            dc_list = uptime_status.objects.values_list("DataCenter", flat=True).distinct("DataCenter").order_by("DataCenter").filter(status="down")
    ...
    def get_acknowledgement_status(self, *args, **kwargs):
        ...
        try:
            get_sid_inst = uptime_time_series_data.objects.get(SID=kwargs['SID'], ToTime=None, Status="down")
