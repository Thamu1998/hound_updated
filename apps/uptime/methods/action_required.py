...
log = init_logging("logs/uptime_name_change.log", __name__)
############### END Initiate logger #############################
...
class action_required_data(object):
    ...
    def get_data(self,**kwargs):
        ...
        dc_mismatch_count = uptime_status.objects.exclude(dc_info="NA").exclude(DataCenter=F('system_info__DataCenterID')).annotate(correct_dc=(F('system_info__DataCenterID'))).exclude(correct_dc=None).count()
