$(document).on('click', '.sid_btn', function () {
    ...
    $.ajax({
      method: "GET",
      url: `/uptime/view/comment?SID=`+sid,
      success: function(data){
        if(data.length > 0){
...
function generate_acknowledge_btn(sid){

  $.ajax({
    method: "GET",
    url: `/uptime/acknowledge/downtime/system?SID=`+sid,
    success: function(data){
      generate_acknowledge_btn_init(data);
    },
    error: function(error_data){
  ...
  function generate_acknowledge_btn_init(data){
