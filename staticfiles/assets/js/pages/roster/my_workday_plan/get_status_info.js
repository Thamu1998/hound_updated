$(document).on('click', '.view_shift_status_change_btn', function (e) {
    
    var sdate = $(this).attr('sdate');

    var cshift = $(this).attr('cshift');

    $('#create_shift_status_request_btn').attr('sdate', sdate);
    $('#create_shift_status_request_btn').attr('cshift', cshift);

    $.ajax({
        method: "GET",
        url: '/roster/member/shift/info?sdate='+sdate+'&cshift='+cshift,
        success: function(data){
            generate_status_request_info_dom(data);
        },
    ...
    $.ajax({
        ...
        error: function(error_data){
            $('#kt_status_request_info_acct').html("");
            console.log(JSON.parse(error_data.responseText))
        }
    });
...
function generate_status_request_info_dom(data){
    ...
    $('#status_change_shift_member_div').html(member_list_div)
    
};

function generate_status_request_cmd_dom(data, sdate, cshift) {
    ...
    var comment_textarea_card = `<div class="card-footer p-0 m-0" id="kt_chat_messenger_footer">
                                    <div class="input-group input-group-solid">
                                        <textarea class="form-control" id="comment_status_text_box" placeholder="Type a message" aria-label="With textarea"></textarea>
                                        <button request_id="`+data.request_id+`" sdate="`+sdate+`" cshift="`+cshift+`" class="btn btn-light-primary" id="update_status_command_btn" type="button" data-kt-element="send">Send</button>
                                    </div>
    ...
    $('#kt_status_request_info_acct').html(accord);
}