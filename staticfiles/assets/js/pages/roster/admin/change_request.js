function get_change_request()
{
    $.ajax({
        method: "GET",
        url: '/roster/admin/change/request/list',
    ...
    function generate_change_request_list_dom(data){
        ...
        data.forEach(function(item){
            ...
            table_row = table_row + `<tr align="center">
                                      ...
                                      <td><p>`+item.sdatef+`</p><span class="badge py-3 px-4 fs-7 badge-light-dark">`+item.change_from__name+`</span>&nbsp;<i class="las la-arrow-right fs-2"></i>&nbsp;<span class="badge py-3 px-4 fs-7 badge-light-primary mt-5">`+item.change_to__name+`</span></td>
        ...
        if (table_row == ""){
            table_row = `<tr><td align="center" valign="middle" colspan=3>No Change Requests</td></tr>`
        }

        $('#change_request_table_body_div').html(table_row);
