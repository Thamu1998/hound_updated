...
var KTDatatablesSearchOptionsAdvancedSearch = function() {
	...
	$.fn.dataTable.Api.register('column().title()', function() {
		// console.log("KTDATA",$(this.header()).text().trim())
		return $(this.header()).text().trim();

	}); 	
	console.log("outside init table")
...
// 	var URL = .replace(/&amp;/g, '&')+`EndDateTime_gt=`+end_date_start+`&EndDateTime_lt=`+end_date_end;
// }else{
...
var colums = [
				{ data: 'VulnameKey',name: 'Vulnerablity Name'}
...
rowGroup_data = {dataSrc: 'VulnameKey',startRender: function(rows, group) {
	return $('<tr class="group group-start"><td class="bg-secondary text-dark" colspan="28"></td></tr>');
}},
console.log(URL,"url")
// KTDatatablesSearchOptionsAdvancedSearch.init(URL, columnDefs, columns_data, columnDefs, sort_data, rowGroup_data, default_query_filter)
// initTable(URL, columnDef_data, columns_data, columnDefs_data, sort_data, rowGroup_data, default_query_filter)
