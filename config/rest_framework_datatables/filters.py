            ...
            if f_search_value:
                if f_search_regex:
                    if self.is_valid_regex(f_search_value):
                        ...
                        temp_q = Q()
                        for x in f['name']:
                            get_multiple_search = f_search_value.split(';')
                            for ser_val in get_multiple_search:
                                if ser_val.endswith('*') or ser_val.startswith('*'):
                                    temp_q |= Q(**{'%s__iregex' % x: ser_val})
                                else:
                                    temp_q |= Q(**{'%s__iexact' % x: ser_val})
